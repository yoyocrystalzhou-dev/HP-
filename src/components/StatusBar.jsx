...
export default function StatusBar({ player, variant = "rail", uiMode = "night", section = "all", onClose, ocs = [], clues = [], houseCup = null, onAddOc, onRemoveOc, favorList = [], onRestart }) {
  ...
  const showStatus = showAll || section === "status";
  const showContribution = showAll || section === "contribution";
  const showInventory = showAll || section === "inventory";
  const showCourses = showAll || section === "courses";
  const showFavor = showAll || section === "favor";
  ...
  return (
    <div style={shell}>
      ...
      {/* 养成数值条 */}
      {showStatus && <div style={{ padding: isSheet ? "10px 20px" : "10px 16px", display: isSheet ? "grid" : "flex", gridTemplateColumns: isSheet ? "1fr 1fr" : undefined, gap: isSheet ? "12px 18px" : 11, flexDirection: isSheet ? undefined : "column", flex: isSheet ? undefined : 1 }}>
        {STAT_DEFS.map((d) => {
          const v = s[d.key] ?? 0;
      ...
      {showStatus && meta.subjects?.length > 0 && (
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 6, marginBottom: 4 }}>
                    <span style={{ fontSize: 12.5, color: C.ink, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {f.name}{f.kind === "oc" ? <span style={{ color: C.gold, fontSize: 9.5, marginLeft: 4, fontWeight: 700 }}>OC</span> : null}
                    </span>
                    <span style={{ flexShrink: 0, fontSize: 10.5, fontWeight: 700, color: isLover ? "#c2557a" : C.gold }}>
                      {isLover ? "❤ " : ""}{stage} · {Math.round(f.value)}
                    </span>
                  </div>
